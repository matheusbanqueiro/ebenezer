    ...
    return (
      <div className={`relative ${full ? "w-full" : ""}`}>
        <input
          ...
          type={inputType}
          className={cn(
            `flex h-10 w-full rounded-md bg-slate-100 ring-2 ring-slate-200 ring-offset-0 focus:outline-none focus:ring-2 focus:ring-slate-500/80 px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-slate-400 disabled:cursor-not-allowed disabled:opacity-50`,
            className
          )}
          ref={ref}
          {...props}
